...
exports.profile = async (req, res) => {
    logger.info(`${loggingPrefix} Profile route hit - /users/profile/:id`);
    const userId = parseInt(req.params.id, 10);

    if (isNaN(userId)) {
        logger.warn(`${loggingPrefix} Invalid user ID:`, req.params.id);
        return res.status(400).send('User ID is required and must be a number');
    }

    try {
        logger.info(`${loggingPrefix} Looking up user ID:`, userId);
        ...
        if (!user) {
            logger.warn(`${loggingPrefix} User not found for ID:`, userId);
        ...
        const stories = await db
            ...
            .groupBy([
                's.id', 's.title', 's.synopsis', 's.created_at', 's.updated_at'
            ])
...
exports.view = (req, res) => {
    logger.info(`${loggingPrefix} View route hit - /users/:id`);
